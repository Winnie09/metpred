...
pd1 <- readRDS(paste0(pddir, 'perf_fdrdiff_cutoff1.rds')) ######### caution
pd2 <- readRDS(paste0(pddir, 'perf_auc_cutoff1.rds')) ######### caution
...
p2 <- ggplot(data = pd2, aes(x = method, y = performance, fill = method)) + 
  ...
  scale_fill_brewer(palette = 'Set1')
library(gridExtra)
pdf(paste0(pdir, 'perf_method_cutoff1.pdf'), width = 5, height = 2.2)
