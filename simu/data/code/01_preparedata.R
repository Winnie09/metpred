...
fullmat <- readRDS('/home-4/whou10@jhu.edu/scratch/Wenpin/metpred/nullsimu/data/data/4To4_allcell/pred_ery.rds')
ap <- as.numeric(sub('BM','',sub(':.*', '', colnames(fullmat))))
set.seed(12345)
id1 <- sample(which(ap %in% c(1,2,5,6)), numcell1)
id2 <- sample(which(!ap %in% c(1,2,5,6)), numcell2)
...
## save files
setwd('/home-4/whou10@jhu.edu/scratch/Wenpin/metpred/simu/data/data/')
