...
for (s in 1:ncol(d)){
  ...
  pdf(paste0(pdir, 'Example_sample_across_CpGs_', s, '.pdf'), width = 3.3, height = 2.6)
  print(ggplot(data2, aes(x=x, y=y) ) +
          ...
          theme_classic() +
          xlab('Measured DNAme')+
          ylab('Predicted DNAme')+
	  ggtitle(paste0('PCC=', round(cor(data2[,1], data2[,2]),2))))
  dev.off()  
}
...
for (cpg in sample(rownames(c.sub), 10)){
  ...
  pdf(paste0(pdir, paste0('Example_CpG_across_samples_', cpg, '.pdf')), width = 2.6, height = 2.6)
