...
predme <- readRDS('/home/whou10/data/whou/metpred/data/gtex_wgbs/processed/wgbs.rds')
int <- intersect(rownames(me),rownames(predme))
me <- me[int,]
predme <- predme[int,]
predme <- predme[,colnames(predge)]

source('/home/whou10/data/whou/metpred/software/trainpredict.R')
pred <- trainpredict(trainexpr=ge,testexpr=predge,trainmeth=me)
saveRDS(pred,file='/home/whou10/data/whou/metpred/evaluate/encode_gtex/pred/permu.rds')
