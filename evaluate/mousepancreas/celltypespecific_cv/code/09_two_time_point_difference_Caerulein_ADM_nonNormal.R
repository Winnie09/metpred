...
int = Reduce(intersect, list(rownames(pred), rownames(pred1.2), rownames(pred2), rownames(pred2.2),rownames(pred3)))
str(int)

pred.all = cbind(pred[int, ,drop=F], pred1.2[int,,drop=F], pred2[int,,drop=F], pred2.2[int,,drop=F], pred3[int,,drop =F])
str(pred.all)
ct = sub('.*:','',colnames(pred.all))
# pred.acinar.diff = pred.all[,ct=='acinar'] %*% matrix(c(-1/2, -1/2, 1,0,0,0, -1,1), ncol = 2)
pred.duct.diff = pred.all[,ct=='duct'] %*% matrix(c(-1/2, -1/2,1/2, 1/2,0,0,0, -1/2,-1/2,1), ncol = 2)
