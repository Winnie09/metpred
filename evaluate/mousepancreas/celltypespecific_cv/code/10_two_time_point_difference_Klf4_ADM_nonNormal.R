...
s1 = meta[meta[,2] == 'Klf4_ADM' & meta[,'Phenotype'] != 'Normal' & meta[,3] == 'Day 2', 1]
s2 = meta[meta[,2] == 'Klf4_ADM' & meta[,'Phenotype'] != 'Normal' & meta[,3] == 'Day 4', 1]
s3 = meta[meta[,2] == 'Klf4_ADM' & meta[,'Phenotype'] != 'Normal' & meta[,3] == 'Day 7', 1]
...
str(pred)
pred = do.call(rbind, pred) # 19634603 CpG
...
allf2 = list.files((paste0('/home/whou10/data/whou/metpred/evaluate/mousepancreas/celltypespecific_cv/res/', s2[1])))
allf2
pred2 <- lapply(allf2, function(f){
  tmp = readRDS(paste0('/home/whou10/data/whou/metpred/evaluate/mousepancreas/celltypespecific_cv/res/', s2[1], '/',f))
})
str(pred2)
pred2 = do.call(rbind, pred2)


allf2.2 = list.files((paste0('/home/whou10/data/whou/metpred/evaluate/mousepancreas/celltypespecific_cv/res/', s2[2])))
allf2.2
pred2.2 <- lapply(allf2.2, function(f){
  tmp = readRDS(paste0('/home/whou10/data/whou/metpred/evaluate/mousepancreas/celltypespecific_cv/res/', s2[2], '/',f))
})
pred2.2 = do.call(rbind, pred2.2)
str(pred2.2)


allf3 = list.files((paste0('/home/whou10/data/whou/metpred/evaluate/mousepancreas/celltypespecific_cv/res/', s3[1])))
allf3 ## check job completeness
pred3 <- lapply(allf3, function(f){
  tmp = readRDS(paste0('/home/whou10/data/whou/metpred/evaluate/mousepancreas/celltypespecific_cv/res/', s3[1], '/',f))
})
pred3 = do.call(rbind, pred3)
str(pred3)

allf3.2 = list.files((paste0('/home/whou10/data/whou/metpred/evaluate/mousepancreas/celltypespecific_cv/res/', s3[2])))
allf3.2 ## check job completeness
pred3.2 <- lapply(allf3.2, function(f){
  tmp = readRDS(paste0('/home/whou10/data/whou/metpred/evaluate/mousepancreas/celltypespecific_cv/res/', s3[2], '/',f))
})
pred3.2 = do.call(rbind, pred3.2)
str(pred3.2)


int = Reduce(intersect, list(rownames(pred), rownames(pred1.2), rownames(pred2), rownames(pred2.2),rownames(pred3), rownames(pred3.2)))
str(int)

pred.all = cbind(pred[int, ,drop=F], pred1.2[int,,drop=F], pred2[int,,drop=F], pred2.2[int,,drop=F], pred3[int,,drop =F], pred3.2[int,,drop=F])
str(pred.all)
ct = sub('.*:','',colnames(pred.all))
# pred.acinar.diff = pred.all[,ct=='acinar'] %*% matrix(c(-1/2, -1/2, 1,0,0,0, -1,1), ncol = 2)
pred.duct.diff = pred.all[,ct=='duct'] %*% matrix(c(-1/2, -1/2,1/2, 1/2,0,0,0,0, -1/2,-1/2,1/2,1/2), ncol = 2)
...
gs.duct <- w[int, paste0(c(s1,s2,s3),':duct')] %*% matrix(c(-1/2, -1/2,1/2, 1/2,0,0,0,0, -1/2,-1/2,1/2,1/2), ncol = 2)
