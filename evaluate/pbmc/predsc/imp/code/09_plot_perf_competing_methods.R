...
p1 <- data.frame(method='Nearest gene',type=type,readRDS(paste0(rdir, 'neargene/', key, '_across',type,'_pcc.rds')),stringsAsFactors = F)
p2 <- data.frame(method='Permute',type=type,readRDS(paste0(rdir,'permu/', key, '_across',type,'_pcc.rds')),stringsAsFactors = F)
p3 <- data.frame(method='Ramp',type=type,readRDS(paste0(rdir, 'ramp/', key, '_across',type,'_pcc.rds')),stringsAsFactors = F)
pd <- rbind(p1,p2,p3)
...
pdf(paste0(pdir, key, '_acrosscpg_pcc_allmethod.pdf'),width=3.2,height=1.5)
ggplot(pd,aes(x=method,y=cor,fill=method)) + 
  geom_violin(alpha = 0.1, scale = 'area') + 
...
saveRDS(apd, paste0(pdir, key, '_across_sample_plotdata.rds'))

pdf(paste0(pdir, key, '_acrosssample_pcc_allmethod.pdf'),width=3.5,height=2.8)
