...
d <- sapply(c(1:10),function(i) {
  d <- readRDS(paste0('/home/whou10/data/whou/metpred/evaluate/encode_newct/pred/res/',type,'/',i,'.rds'))
...
rdir2 = '/home/whou10/data/whou/metpred/evaluate/encode_newct/perf/res/'
v_median <- sample <- cpg <- NULL
for (i in 1:sid) {
  sample <- rbind(sample,data.frame(cor=d[[i]]$sample_cor,rmse=d[[i]]$sample_rmse,cvid=i,stringsAsFactors = F))
  df <- data.frame(cor=d[[i]]$cpg_cor,rmse=d[[i]]$cpg_rmse,sd=d[[i]]$cpg_sd,cvid=i,stringsAsFactors = F)
  saveRDS(df,file=paste0(rdir2,type,'/acrosssample/cv', i, '.rds'))
  v_median <- c(v_median, median(d[[i]]$cpg_cor, na.rm = TRUE))
  cpg <- rbind(cpg,df)
}
rdir = '/home/whou10/data/whou/metpred/evaluate/encode_newct/perf/res/'
saveRDS(sample,file=paste0(rdir,type,'/all/sample.rds'))
saveRDS(cpg,file=paste0(rdir,type,'/all/cpg.rds'))
names(v_median) <- 1:sid
saveRDS(v_median,file=paste0(rdir,type,'/cvmedian/cpg.rds'))
...
#   },simplify = F))
# },simplify = F))
# saveRDS(difcor,file=paste0(rdir, type,'/all/difcor.rds'))
