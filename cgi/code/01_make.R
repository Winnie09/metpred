...
tb = read.table('/home-4/whou10@jhu.edu/work-zfs/whou10/metpred/data/cpg_island/hgTables.txt', stringsAsFactors = FALSE)
library(GenomicRanges)
gr <- GRanges(seqnames=tb[,2],IRanges(start=tb[,3],end=tb[,4]))

## read in predicted CpG location information of cancer samples
pred <- readRDS('/home-4/whou10@jhu.edu/scratch/Wenpin/metpred/predict/res/predicted_DNAm_on_testset_cancer.rds')
rn <- rownames(pred)
start <- as.numeric(sub('.*_', '', rn))
cpg <- GRanges(seqnames= sub('_.*', '', rn), IRanges(start=start,end=start))
...
## read in cancer samples measured DNA methylation (true values)
...
pdir <- '/home-4/whou10@jhu.edu/scratch/Wenpin/metpred/cgi/plot/'
rdir <- '/home-4/whou10@jhu.edu/scratch/Wenpin/metpred/cgi/res/'
...
pdf(paste0(pdir, 'cgi_mean_violin.pdf'), width = 3.3, height = 1.854)
ggplot(data = pd, aes(x = type, y = value, fill = type)) + geom_violin(scale = 'width', width = 0.8, alpha = 0.3) + geom_boxplot(outlier.shape = NA, width = 0.1) + scale_color_brewer(palette = 'Set2') + theme_classic() + theme(legend.position = 'none', axis.text = element_text(color = 'black', size = 10), axis.title = element_text(color = 'black', size = 10))
dev.off()
