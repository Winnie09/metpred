...
for (org in c('GRCh38','mm10')) {
  af <- meta[which(meta$Organism==conv[org]),'file']
  cc = rep("NULL", 14)
  cc[c(1,6)] <- 'character'
  cc[c(2,11)] <- 'numeric'
  d <- sapply(af,function(f) {
		      print(f)
    d <- fread(paste0('/home/whou10/data/whou/metpred/data/encode/wgbs/bed/',f,'.bed.gz'),data.table=F,colClasses=cc)
    v <- d[,4]/100
  ...
  saveRDS(m,file=paste0('/home/whou10/data/whou/metpred/data/encode/wgbs/proc/',org,'.rds'))
}
