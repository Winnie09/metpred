...
d <- log2(d + 1)
saveRDS(d, '/home/whou10/data/whou/metpred/data/hsc/proc/rna_agg_sample.rds')
saveRDS(w, '/home/whou10/data/whou/metpred/data/hsc/proc/wgbs_agg_sample.rds')

int <- intersect(colnames(d), colnames(w))
d <- d[,colnames(d) %in% int]
w <- w[,colnames(w) %in% int]
