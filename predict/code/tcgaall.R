...
int <- intersect(rownames(d),rownames(gd))
d <- d[int,]
gd <- gd[int,]

scalematrix <- function(data) {
  cm <- rowMeans(data,na.rm=T)
  rs <- rowSums(!is.na(data))
  csd <- sqrt((rowMeans(data*data,na.rm=T) - cm^2) / (rs - 1) * rs)
  (data - cm) / csd
}

corfunc <- function(m1,m2,type='concordant') {
  tmp <- scalematrix(m1) * scalematrix(m2)
  rowSums(tmp,na.rm=T)/(rowSums(!is.na(tmp))-1)
}

sfunc <- function(tmpm) {
  cm <- rowMeans(tmpm,na.rm=T)
  rs <- rowSums(!is.na(tmpm))
  csd <- sqrt((rowMeans(tmpm*tmpm,na.rm=T) - cm^2) / (rs - 1) * rs)
